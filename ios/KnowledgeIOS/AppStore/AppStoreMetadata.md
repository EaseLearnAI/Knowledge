...
你的收藏、Tag、搜索与对话默认保存在设备上。Memo 不要求账号，不接入广告、跨 App 追踪或第三方分析 SDK。若设备支持并启用了 Apple Intelligence，Memo 可使用 Apple 的端侧模型；否则使用本地可追溯的文本提取算法。
...
- App 不要求账号或登录。
- 主要流程：打开 App → 点击添加 → 粘贴任意公开 HTTPS 网页 → 等待处理完成 → 查看摘要、Tag、搜索或提问。
...
选择：**Data Not Collected（不收集数据）**。

说明：

- 内容和对话仅保存在用户设备。
- 用户主动收藏网页时，设备会直接请求来源网站；Memo 没有自有内容服务器。
- 不包含账号、广告、分析、追踪或第三方数据 SDK。
