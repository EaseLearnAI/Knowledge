...
把值得记住的网页扔进来。Memo 自动提取正文、生成摘要和 Tag，让你随时找得到，也能基于自己的收藏直接问 AI。

## 完整描述

Memo 是一个本地优先的内容收藏与知识问答 App。
...
- 自动推荐 Tag，也支持手动调整
- 全文搜索标题、摘要、正文和 Tag
- 基于已收藏的原文回答问题，并附上引用来源

完成产品引导并使用手机号或邮箱注册、登录后，你的收藏、Tag、搜索与对话仍默认保存在设备上。Memo 不接入广告、跨 App 追踪或第三方分析 SDK。若设备支持并启用了 Apple Intelligence，Memo 可使用 Apple 的端侧模型；否则使用本地可追溯的文本提取算法。
...
- 主要流程：产品引导 → 手机号或邮箱注册/登录 → 点击添加 → 粘贴任意公开 HTTPS 网页 → 等待处理完成 → 查看摘要、Tag、搜索或提问。
- 测试链接可使用：`https://example.com`
- AI 回答只基于设备中已收藏的内容，并展示引用。
- 无付费墙、订阅或应用内购买。
- 无广告、追踪或第三方分析 SDK。
...
- 登录令牌保存在系统 Keychain，内容和对话仍保存在用户设备。
