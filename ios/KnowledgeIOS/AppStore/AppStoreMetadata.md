...
使用手机号或邮箱注册、登录并完成两页产品引导后，你的收藏、摘要、Tag 与搜索索引仍默认保存在设备上。Memo 不接入广告、跨 App 追踪或第三方分析 SDK。若设备支持并启用了 Apple Intelligence，Memo 可使用 Apple 的端侧模型进行内容分析；否则使用本地可追溯的文本提取算法。
...
- App 首次启动先展示产品介绍，再要求使用手机号或邮箱注册、登录；登录成功后展示两页新手引导。登录令牌保存在系统 Keychain。
- 主要流程：产品介绍 → 手机号或邮箱注册/登录 → 两页新手引导 → 点击添加 → 粘贴受支持的视频链接 → 等待处理完成 → 查看摘要、关键观点、Tag 或搜索。
